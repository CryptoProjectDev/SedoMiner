...
// get the path to the app data folder, which is %LocalAppData%\SEDOminer on Windows, and HOME/.config/tokenminer elsewhere. 
...
filesystem::path getAppDataFolder(void)
{
	...
	filesystem::path s(".config/SEDOminer");
#endif
	s = local / s;
...
void keccak256_SEDO(bytes& _challenge, h160& _sender, h256& _nonce, bytes& _hash) {
	bytes mix(84);
