...
public:
	...
	*----------------------------------------------------------------------------------*/
	void execute()
	{
		...
		m_userAcct = ProgOpt::Get("SEDO", "EthAddress");
		u256 mAcctNum;
		try
		{
		...
		{
			LogS << "'EthAddress' contains invalid characters in SEDOminer.ini";
			exit(0);
		}
		if (m_userAcct.substr(0, 2) != "0x")
		{
			LogS << "Invalid 'EthAddress' in SEDOminer.ini - Miner account should start with '0x'";
			exit(0);
		}
		if (mAcctNum == 0 || m_userAcct.length() != 42)
		{
			LogS << "Invalid 'EthAddress' in SEDOminer.ini";
			exit(0);
		}

		string mps = ProgOpt::Get("SEDO", "MinutesPerShare","Pool");
		LowerCase(mps);
		if (mps == "pool")
			m_minutesPerShare = -1;
		else
	...
	*----------------------------------------------------------------------------------*/
	static void streamHelp(ostream& _out)
	{
		_out
			...
			<< "                           if not there, then in 2) %LocalAppData%/SEDOminer/SEDOminer.ini (Windows) or " << endl
			<< "                           $HOME/.config/SEDOminer/SEDOminer.ini (Linux).  If this option is specified,  it must appear " << endl
...
private:
	...
	*----------------------------------------------------------------------------------*/
	void doFarm(GenericFarm<EthashProofOfWork>& f, string _nodeURL, string _rpcPort)
	{
		...
		bool gasPriceBidding = ProgOpt::Get("SEDO", "GasPriceBidding", "0") == "1" && m_opMode == OperationMode::Solo;
		...
		if (m_opMode == OperationMode::Pool)
		{
			nodeClient = new jsonrpc::HttpClient("https://mainnet.infura.io/f12d6274997840158b99b418f0ed8ec1:8545");
			//nodeClient = new jsonrpc::HttpClient("https://ropsten.infura.io/f12d6274997840158b99b418f0ed8ec1:8545");
		...
		while (!m_shutdown)
		{
			try
			{
				...
				keccak256_SEDO(challenge, sender, solution, hash);
				if (h256(hash) < target) {
					if (m_opMode == OperationMode::Pool)
					{
	...
	*----------------------------------------------------------------------------------*/
	void doStratum(GenericFarm<EthashProofOfWork>& f, string _nodeURL, string _rpcPort, string _stratumPort)
	{
		...
		jsonrpc::HttpClient rpcClient("https://mainnet.infura.io/f12d6274997840158b99b418f0ed8ec1:8545");
		//jsonrpc::HttpClient rpcClient("https://ropsten.infura.io/f12d6274997840158b99b418f0ed8ec1:8545");
